...
final class DbGroupedSelection<Row, Key> {
  ...
  Stream<List<DbRow>> watch() =>
      _query._source._watch<List<DbRow>>(load: get, equals: _groupRowsEqual);
  ...
  bool _groupRowsEqual(List<DbRow> left, List<DbRow> right) {
    if (left.length != right.length) return false;
    for (var index = 0; index < left.length; index += 1) {
      final leftValues = left[index].asMap;
      final rightValues = right[index].asMap;
      if (leftValues.length != rightValues.length) return false;
      for (final entry in leftValues.entries) {
        if (rightValues[entry.key] != entry.value ||
            !rightValues.containsKey(entry.key)) {
          return false;
        }
      }
    }
